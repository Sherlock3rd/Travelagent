...
目的地、日期、出发地、同行人员数量及偏好由用户后续补充。当前五模块已实现本机编辑，没有创建云端业务表或虚构示例行程。

## Supabase 接入

本机 `.env` 已配置独立 Travelagent 项目；换设备时将 `.env.example` 复制为 `.env`，填写项目 URL 与 publishable key，再执行 `npm.cmd run check:cloud`。脚本仅检查 Auth settings API 连通性，不读写业务数据，不输出密钥，也不表示业务同步已实现。`.env` 不提交到 Git。
