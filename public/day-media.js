...
export const PHOTO_HOSTS = ['egymonuments.gov.eg', '1442038683.rsc.cdn77.org', 'dohahamadairport.com', 'upload.wikimedia.org', 'thumb.wikimedia.org', 'orange-bay.tours'];
...
export function activityPhotos(activity) { return [activity.photo, ...(activity.photos || [])].filter(Boolean); }
export function dayMediaHTML(day, esc) {
  const seen=new Set();
  const groups=day.activities.map(a=>({a,photos:activityPhotos(a)})), photos=[];
  for(let i=0;i<9;i++) for(const group of groups) {const photo=group.photos[i];if(photo && !seen.has(photo.url)){seen.add(photo.url);photos.push({a:group.a,photo});}}
  return '<section class="day-media'+(photos.length?'':' map-only')+'" data-media-day="'+day.id+'" aria-label="当天路线与景观"><div class="day-map-panel"><div class="day-media-heading"><h3>当日路线</h3><button class="text-button" data-mini-open="'+day.id+'">展开地图 ↗</button></div>'+
    (day.routePlans.length?'<label class="mini-plan-label"><span class="sr-only">路线方案</span><select data-mini-plan="'+day.id+'">'+day.routePlans.map(p=>'<option value="'+p.id+'"'+(p.id===previewSelection.get(day.id)?' selected':'')+'>'+esc(p.name)+'</option>').join('')+'</select></label>':'')+
    '<div class="mini-map-wrap"><div class="daily-mini-map" id="mini-'+day.id+'" aria-label="当日局部路线地图"><span class="mini-placeholder">地图加载中</span></div></div><div class="mini-tools"><button class="text-button" data-mini-fit="'+day.id+'">适应路线</button><button class="text-button" data-mini-pois="'+day.id+'">聚焦景点</button><span class="mini-key" title="实线为道路估算；虚线为顺序或航段示意">━ 道路　┄ 示意</span></div><div class="mini-stop-list" id="mini-stops-'+day.id+'"></div></div>'+
    (photos.length?'<div class="day-photo-panel" data-photo-gallery><div class="day-media-heading"><h3>沿途风景</h3><span class="photo-total">'+photos.length+' 张参考照片</span></div><div class="day-photo-grid photos-'+Math.min(photos.length,4)+'">'+photos.map(({a,photo},i)=>'<figure'+(i>=4?' hidden':'')+'>'+photoButton(photo,esc)+(i===3 && photos.length>4?'<span class="gallery-more">共 '+photos.length+' 张 · 点击浏览</span>':'')+'<figcaption><button class="text-button" data-go-event="'+a.id+'" data-day="'+day.id+'">'+esc(photo.title)+' ↗</button><span class="photo-status">'+(a.status==='optional'?'备选':a.status==='confirmed'?'已确认安排':'计划中')+'</span></figcaption></figure>').join('')+'</div></div>':'')+'</section>';
}
...
export function mountDayMedia(root, days, esc, onEvent, onOpen) {
  ...
  function fit(id,pois=false) {
    ...
    const photoPoints=day.activities.filter(a=>activityPhotos(a).length && (!g.planId || g.stops.some(s=>s.eventIds?.includes(a.id)))).map(a=>a.point || g.stops.find(s=>s.eventIds?.includes(a.id))?.point).filter(Boolean);
  ...
  root.querySelectorAll('.scenery-image img').forEach(img=>img.onerror=()=>{img.hidden=true;img.parentElement.classList.add('image-unavailable');img.parentElement.querySelector('span').textContent='图片暂未加载 · 点击重试';});
